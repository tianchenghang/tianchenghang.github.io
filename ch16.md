...
1. 如果 (表面) 抛出一个对象, 则 (实际) 返回的 promise 的 PromiseState 为 rejected, PromiseResult 是抛出对象
2. 如果 (表面) 返回非 Promise 对象, 则 (实际) 返回的 promise 的 PromiseState 为 fulfilled, PromiseResult 是返回值
3. 如果 (表面) 返回 Promise 对象, 则 (实际) 返回该 Promise 对象
4. 如果 onfulfilled, onrejected 回调函数中 throw what, 则 then, catch 返回 Promise{ PromiseState: rejected, PromiseReturn: what }
5. 如果 onfulfilled, onrejected 回调函数中 return what (没有 return 等价于 return undefined), 则 then, catch 返回 Promise{ PromiseState: fulfilled, PromiseReturn: what }
...
- async 函数总是返回一个 Promise 对象, await 只能用在 async 函数体中
- async 函数 (表面) 返回一个非 Promise 对象 obj, (实际) 返回 `Promise{ PromiseState: 'fulfilled',
PromiseResult: obj }`
