...
```js
const target = { foo: 1, __proto__: { bar: 2 } };
const op = new Proxy(target, {
  ownKeys: function () {
    return ["bar" /* 继承的属性 */];
  },
});
...
ownKeys() 返回的数组中的元素类型, 只能是字符串或 Symbol
...
preventExtensions() 返回一个布尔值, 表示阻止扩展成功或失败

只有目标不可扩展时, 即 `Object.isExtensible(target) === false` 时, preventExtensions() 才能返回 true, 表示阻止扩展成功
...
console.log(Object.getPrototypeOf(target) === proto); // true
```
