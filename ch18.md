...
function* bar() {
  yield 3;
  for (let item of foo() /* Generator, implements IterableIterator */) {
    yield item;
  }
  yield 4;
}
...
生成器函数 GeneratorFunction 返回一个生成器 Generator, 生成器实现了 IterableIterator 接口, 生成器是生成器函数的实例
...
- thunk 函数: 将多参数函数 (包含 callback) 转换为单 callback 参数的 thunk 函数
...
const thunk = Thunk(fs.readFile);
const readFileThunk = thunk("./package.json", "utf-8");
